...
evaluate.logging.set_verbosity(1)


def _parse_args():
    parser = argparse.ArgumentParser(description='PyTorch Image Segementation')
...
def main():
    ...
    dataset_train = CropDataset(root=data_dir, is_train=True, transform=transform_train)
    dataset_val = CropDataset(root=data_dir, is_train=False, transform=transform_val)
    ...
    if args.optim == 'sgd':
        ...
        optimizer = torch.optim.AdamW(params_to_optimize, lr=args.lr_base, weight_decay=args.weight_decay)
    else:
    ...
    metrcis_best, losses, ious = \
        train_model(model, loss_fn, optimizer, scheduler, dataloaders, train_sampler, device, args)
    
    with open(os.path.join('metrics_best.txt'), 'w') as f:
        f.write(str(metrcis_best))
    print('-' * 30)
...
def train_model(
    ...
    for epoch in range(args.epochs):
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'train':
                metric_logger.update(loss_train=epoch_loss, iou_train=iou, lr=optimizer.param_groups[0]["lr"])
            else:
                metric_logger.update(loss_val=epoch_loss, iou_val=iou, lr=optimizer.param_groups[0]["lr"])

            loss_avg = metric_logger.meters['loss_' + phase].avg * args.world_size
            iou_avg = metric_logger.meters['iou_' + phase].avg
            losses[phase].append(loss_avg)
            ious[phase].append(iou_avg)

            if phase == 'train':
                print(f'Epoch [{epoch+1:4d}/{args.epochs:4d}]   Train Loss: {loss_avg:.3e}, mIOU: {iou_avg:.4f}', end='')
            else:
                print(f'   Val Loss: {loss_avg:.3e}, mIOU: {iou_avg:.4f}   Time: {(time.time()-t1):.0f}s')

            # save best model
            if phase == 'val' and iou_avg > iou_best:
                iou_best = iou_avg
    ...
    if dist.get_rank() == 0:
        torch.save(best_model_weights, args.exp_dir + f'model_jac_{iou_best}.pth')

    time_elapsed = time.time() - t_start
    print(f'Training completed in {time_elapsed // 60:.0f} min {time_elapsed % 60:.0f} s')
    print(f'Best Validation Jaccard: {iou_best:.4f}')

    return metrics_best, losses, ious
...
def plot_loss_and_acc(train_losses, train_accs, val_losses, val_accs, out_pth="model.pth"):
    ...
    axs[1].legend(["Train mIOU", "Val mIOU"])
